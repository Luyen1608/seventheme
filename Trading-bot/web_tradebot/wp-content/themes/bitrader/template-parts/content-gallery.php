...
<?php if (is_single()) : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('blog__item blog-details-wrap format-gallery'); ?>>
...
<?php else : ?>
    ...
    <div class="col-md-6 grid-item">
        <article id="post-<?php the_ID(); ?>" <?php post_class('blog__item format-gallery'); ?>>
            <div class="blog__item-inner blog__item-inner--style2">
                <?php if (!empty($gallery_images)) : ?>
                    <div class="blog__thumb blog-post-thumb-active">
                        <?php foreach ($gallery_images as $key => $image) :  ?>
                            <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>">
                        <?php endforeach; ?>
                    </div>
